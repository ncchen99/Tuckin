...
class ConfirmationTimeoutPage extends StatefulWidget {
  const ConfirmationTimeoutPage({super.key});

  @override
  State<ConfirmationTimeoutPage> createState() =>
      _ConfirmationTimeoutPageState();
}

class _ConfirmationTimeoutPageState extends State<ConfirmationTimeoutPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ...
        debugPrint('ConfirmationTimeoutPage 完全渲染');
      }
    });
  }
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        if (status == 'confirmation_timeout') {
          ...
          debugPrint('用戶狀態不是逾時未確認: $status，導向到適當頁面');
          setState(() {
            _isLoading = false;
          });
  ...
  Future<void> _handleReturnToHome() async {
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        // 操作完成後跳轉到首頁
        if (mounted) {
          debugPrint('用戶確認逾時未確認狀態，導向到首頁');
          _navigationService.navigateToHome(context);
        }
      }
    } catch (e) {
      debugPrint('處理逾時未確認時出錯: $e');
      setState(() {
