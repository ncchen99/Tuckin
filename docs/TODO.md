...
- [x] image cropper
- [ ] 本地暫存功能並且把圖片API請求還有loading的動畫處理寫成一份文件
- [ ] 目前設想取得群組的用戶頭貼應該要寫在dining的API，此API可以一次返回全部群組成員的頭貼URL，這邊可以考慮將頭貼的檔名匿名化(把user id隱藏)
- [ ] 或是這個API還可以返回額外的聚餐資訊也就是用戶的暱稱頭貼還有他的自我介紹一句話
- [ ] 前端制作一個圖片持久化的服務專門用來持久化圖片，跟user的service還有dining service分開
- [ ] 一句話介紹自己時要吸上去避免鍵盤遮擋
- [X] 保存預設頭貼
