...
- [x] [新功能] 參加名單
- [x]  參加名單當中的滑動區域有陰影被遮擋有問題該區域需要擴大可能增加 padding
- [x] [新功能] 聊天室
- [x] 加入日期的顯示
- [x] 加入同一個使用者傳送訊息只顯示一次使用者名稱的設計
- [x] 訊息時間要是根據裝置的時區
- [v] [新功能] 點擊群組聊天室的通知訊息開啟APP需要導航到該聊天室頁面

- [x] 聊天室進入時的預設訊息修改並且修復載入速度
- [X] 想要加大聊天室中頭貼的大小
- [X] 上方的header bar圖片有遮擋的問題，感覺是聊天室的標題高度不夠，需要使這一個聊天室的文字和左方的back按鈕垂直位置對齊
- [x] 修正訊息從上面出來的問題，新訊息應該要顯示在容器內部的下方
- [x] 聊天室圖片沒有快取的問題(可能因為每次網址都不一樣，本地快取的紀錄方式需要修改，參考頭貼的方式)
- [x] 圖片的長寬是不是也可以快取到 SQLite 當中? 這樣子在顯示上可以先預留一個確定的高度增加使用者的體驗 不會載入之後整體的訊息高度改變而有跳動的感覺
- [x] 對方傳送訊息的時間顯示在訊息的右邊(在訊息區塊的外面)，不要在訊息的下方，並且顯示小時和分鐘就好 12 小時制，並且在訊息和訊息之間如果有跨日的話顯示日期(在畫面中左右置中)
- [x] 圖片不顯示邊框但有陰影 點擊之後可以放大，左上角一樣顯示 back按鈕點擊之後可以縮小圖片，或者是使用手勢下滑也可以
- [x] 重新設計輸入框的樣式參考 threads
- [x] 同一個使用者在同一個聊天室內應該使用同一個頭貼(如果該使用者沒有設定頭像的路徑)，SQLite當中記錄並且確保下次開啟也是同一個頭貼
- [x] 應該顯示傳送使用者的名字在訊息框的上面(不在白色區塊內)(圖片也要)
- [x] 下排輸入區域的按鈕點擊應該要有互動效果
- [x] 優化圖片傳送時的效果
- [x] 畫面有變動重新渲染時圖片重新載入的問題(出現 loading 動畫) 包括在real time事件的時候
- [x] 聊天室的投帖圖片快取機制要增強現在看起來會一直重複取得頭貼的照片應該是使用使用者ID去比對就好，聊天室的使用者頭貼目前看起來沒有使用到快取照片
- [x] 背景圖片不要跟著移動
- [x] 聊天室還有參加名單的使用者頭貼取消藍色的邊框直接顯示圓形頭貼就好

- [v] 訊息進來就會跳通知然後頁面就會重整
- [v] 如果已經在聊天室就不顯示通知
- [x] 優化presigned url的請求 新增可以一次請求大量的API
- [x] 優化Cloudflare重複創建實例的問題優化cloudfire建立presigned url的同步問題改成非同步
- [x] 聊天室的header bar和訊息區域交接處要做漸層漸變(不做)
- [x] 修正聊天室的header bar的標題的高度文字顯示的位置對齊 header bar的垂直置中圖片顯示頁面也要
- [x] 送出後應該不要讓介面卡住而是直接顯示在畫面上 並等待資料庫同步 (如果出錯再將前端的顯示刪除 圖片也是一樣需要刪除) 傳送中的訊息可以加入一個狀態顯示傳送中之類的
- [x] 尚未輸入訊息之前送出按鈕應該要是disable的狀態參考
- [x] 在聊天室當中接收到通知為避免realtime沒有運作應該要再次更新訊息
- [x] 等待出席頁面的real time會影響訊息的real time應該要先取消訂閱，在設定的頁面也是一樣
- [x] 訊息頁面的Back和系統back應該都要回到聚餐資訊頁面
- [x] [TODO] 聊天室的訊息群組需要在聚餐結束後清理
- [x] [TODO] 包含前端的裝置的 SQLite
- [x] [TODO] 登出需要清除圖片快取
- [x] 修正說明文字請使用聊天室找到伙伴
- [x] 修正群組預設頭貼問題
- [x] 聚餐結束之后後端清理圖片和聊天訊息
- [x] 評分頁面使用用戶的真實頭像
- [x] 評分頁面直接使用資料庫
- [x] 資料庫的聊天訊息和圖片清理應該是在聚餐當日結束就要清理而不是2天過後[發現可以不用處理]
- [x] 透過通知進入聊天室需要先檢查使用者的狀態如果在waiting_attendance 才可以進入聊天室
- [x] [新功能] 提醒報名聚會
- [x] [新功能] 通知改成早上 9 點發出？
- [x] [新功能] 使用互評加入配對，減少重複的組合
- [x] [API] 修正訊息資料表沒有提醒投票的訊息
- [x] [手動][TODO] 寄信通知已聚餐過或是預約的用戶，服務暫停兩周以上，會在通知他們

- [x] [新功能] 開啟更新提醒，這2個檢查在背景進行不阻塞開啟速度
- [x] [新功能] 服務狀態檢查：系統維運中，目前無法使用，或者是什麼通知?
- [x] 前面這2個功能要顯示的文字應該也是記錄在supabase
- [x] [TODO] 餐廳有顯示圖片的頁面快取都需要修改，Loading的動畫也要修改
- [x] 參加名單在小尺寸螢幕會被遮蔽
- [x] 選單頁面加入可以滑動的效果
- [ ] [TODO] IG 粉專宣傳
...
- [ ] 設定官網CORS
- [ ] 匯出寄信名單
- [ ] 使用 merge for Gmail 寄信
- [ ] [手動][TODO] 之後寄信通知用戶服務啟用，並提醒使用成大信箱登入


** optional **
- [ ] 使用者體驗優化：聊天室在狀態改變時會跳出通知提醒即將刪除聊天室點擊后立即跳轉
...
- [ ] [新功能] 設計餐廳評價功能
- [ ] [新功能] 聚餐歷史數據查詢
- [x] [新功能] 推薦餐廳移除，取消選取時右上角出現一個叉叉
- [x] [新功能] 餐廳移除 API ，需更新table加入包含user ID的欄位
