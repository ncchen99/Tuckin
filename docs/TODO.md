...
- [ ] [新功能] 實作照片上傳的個人資料頁面(是否需要新增預設的選項再考慮)
- [ ] [新功能] 實作照片上傳前的前端壓縮(轉換為web)
- [x] 壓縮到 512 
- [ ] 相機和叉叉的圖案不要使用圓形框而是直接顯示圖片參考input框的圖標顯示
- [X] 保存預設頭貼
- [ ] 選取圖片之后直接顯示在原本的位置 上傳失敗再移除變回預設的
- [x] 同時轉檔和獲取上傳連結
- [x] loading 動畫
- [x] 開啟之后過2秒才顯示性別正確的頭貼
- [ ] 確保上傳和刪除不卡住都在背景運作就好在使用 toast 顯示
- [ ] [TODO] IG 粉專宣傳
