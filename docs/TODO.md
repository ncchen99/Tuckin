...
- [x] [新功能] 聊天室
- [ ] 加入日期的顯示
- [ ] 加入同一個使用者傳送訊息只顯示一次使用者名稱的設計
- [ ] 訊息時間要是根據裝置的時區
- [ ] [新功能] 點擊群組聊天室的通知訊息開啟APP需要導航到該聊天室頁面
- [ ] 聊天室的訊息群組需要在聚餐結束後清理
- [ ] 想要加大聊天室中頭貼的大小
- [ ] 上方的header bar圖片有遮擋的問題，感覺是聊天室的標題高度不夠，需要使這一個聊天室的文字和左方的back按鈕垂直位置對齊
- [ ] 修正訊息從上面出來的問題，新訊息應該要顯示在容器內部的下方
- [ ] 聊天室圖片沒有快取的問題(可能因為每次網址都不一樣，本地快取的紀錄方式需要修改，參考頭貼的方式)
- [ ] 圖片的長寬是不是也可以快取到 SQLite 當中? 這樣子在顯示上可以先預留一個確定的高度增加使用者的體驗 不會載入之後整體的訊息高度改變而有跳動的感覺
- [ ] 對方傳送訊息的時間顯示在訊息的右邊(在訊息區塊的外面)，不要在訊息的下方，並且顯示小時和分鐘就好 12 小時制，並且在訊息和訊息之間如果有跨日的話顯示日期(在畫面中左右置中)
- [ ] 圖片不顯示邊框但有陰影 點擊之後可以放大，左上角一樣顯示 back按鈕點擊之後可以縮小圖片，或者是使用手勢下滑也可以
- [ ] 底部的白色區域要延伸到navigation bar，視覺上設計會比較一致
- [ ] 同一個使用者在同一個聊天室內應該使用同一個頭貼(如果該使用者沒有設定頭像的路徑)，SQLite當中記錄並且確保下次開啟也是同一個頭貼
- [ ] 應該顯示傳送使用者的名字在訊息框的上面(不在白色區塊內)(圖片也要)
- [ ] 下排輸入區域的按鈕點擊應該要有互動效果
